...
app.get("/api/guestbook/:name", async (req, res) => {
    try {
        const { name } = req.params;
        const result = await pool.query(
            "SELECT id, name, message, created_at FROM guestbook WHERE name = $1",
            [name]
        );
        res.json(result.rows);
    } catch (err) {
...
app.put("/api/guestbook/like/:id", async (req, res) => {
    const { id } = req.params;
    try {
        //Q. guestbook 테이블에서 id로 특정 항목의 비밀번호를 가져오세요.
        const result = await pool.query(
            "SELECT like FROM guestbook WHERE id = $1",
            [id]
        );
        if (result.rows.length > 0) {
            const updateResult = await pool.query(
                "UPDATE guestbook SET like = $1 WHERE id = $2 RETURNING id, name, message, created_at, like",
                [!result.rows[0].like, id]
            );
            res.json(updateResult.rows[0]);
        }
    } catch (err) {
